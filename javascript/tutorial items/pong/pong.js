...
class Pong {
    ...
    winner() {
        if (pong.players[1].score >= 7) {
            if (confirm("computer wins! Press ok to restart")) {
                window.location.reload()
            }

            return
        } else if (pong.players[0].score >= 1) {
            window.location = '/projects/pong/index.html'
            
        }
    }

    playerCollideToSides() {
        if (this.players.pos.y >= this._canvas.height) {
            ball.vel.x = -ball.vel.x
        }
    }
